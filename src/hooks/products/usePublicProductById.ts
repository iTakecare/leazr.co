...
import { getPublicProductById } from '@/services/catalogService';
...
export const usePublicProductById = (productId: string | undefined, companyId?: string) => {
  const query = useQuery({
    queryKey: ['public-product', productId, companyId],
    queryFn: async () => {
      if (!productId) return null;
      const productData = await getPublicProductById(productId, companyId);
