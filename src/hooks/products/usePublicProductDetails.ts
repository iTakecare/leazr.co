...
import { usePublicProductById } from './usePublicProductById';
import { Product } from '@/types/catalog';
import { getProductPrice, getMinimumMonthlyPrice } from '@/utils/productPricing';

export const usePublicProductDetails = (productId: string | undefined, companyId?: string) => {
  const { product, isLoading, error } = usePublicProductById(productId, companyId);
  ...
  const currentPrice = useMemo(() => {
    ...
    console.log(`usePublicProductDetails: Current price for ${product.name}:`, priceData.monthlyPrice);
  ...
  const minMonthlyPrice = useMemo(() => {
    ...
    console.log(`usePublicProductDetails: Min monthly price for ${product.name}:`, minPrice);
    return minPrice;
  }, [product]);

  // Calculate total price based on quantity and duration (duration is now fixed)
  const totalPrice = useMemo(() => {
    console.log(`usePublicProductDetails: Calculating total price, currentPrice=${currentPrice}, quantity=${quantity}`);
