...
import { getAmbassadorCommissions, updateAmbassadorCommissionStatus, AmbassadorCommission, calculateTotalAmbassadorCommissions } from "@/services/ambassadorCommissionService";
...
const AmbassadorCommissionsTable: React.FC<AmbassadorCommissionsTableProps> = ({ ambassadorId }) => {
  ...
  const fetchCommissions = async () => {
    try {
      ...
      const commissionsData = await getAmbassadorCommissions(ambassadorIdToUse);
      setCommissions(commissionsData);

      const totalsData = await calculateTotalAmbassadorCommissions(ambassadorIdToUse);
  ...
  const handleUpdateStatus = async (id: string, newStatus: 'pending' | 'paid' | 'cancelled') => {
    try {
      const updated = await updateAmbassadorCommissionStatus(id, newStatus);
      if (updated) {
        setCommissions(prev => 
          prev.map(comm => 
...
export default AmbassadorCommissionsTable;
