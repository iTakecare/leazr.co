...
import { Sparkles, Send, FileText, AlertTriangle, RefreshCw, Megaphone } from "lucide-react";
...
import {
  ...
  getAiReports, generateCmoReport, cmoChat,
} from "@/services/cfoService";

// Markdown enrichi (tableaux, listes) avec styles lisibles pour les rapports CMO
...
const CmoAiTab: React.FC = () => {
  ...
  const load = async () => {
    ...
    try {
      const [r, a] = await Promise.all([
        getAiReports(companyId, "cmo_report"),
        getAiReports(companyId, "cmo_alert"),
      ]);
      setReports(r);
      setAlerts(a);
      if (r.length && !selectedReportId) setSelectedReportId(r[0].id);
    } catch (e) {
      console.error("Erreur chargement rapports CMO:", e);
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true);
    try {
      const report = await generateCmoReport(companyId);
      toast.success("Rapport CMO généré");
  ...
  const handleSend = async () => {
    ...
    setChatting(true);
    try {
      const answer = await cmoChat(companyId, next);
      setMessages([...next, { role: "assistant", content: answer }]);
    } catch (e: any) {
      toast.error(e.message || "Erreur du chat CMO");
  ...
  const alertIsRecent = latestAlert &&
    (Date.now() - new Date(latestAlert.created_at).getTime()) < 7 * 86400000;
  ...
  return (
    <div className="space-y-4">
      {/* Alerte récente */}
      {alertIsRecent && (
        <Alert className="border-fuchsia-300 bg-fuchsia-50">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between gap-2">
              <div>
                <CardTitle className="text-base flex items-center gap-2"><FileText className="h-4 w-4" /> Rapports CMO</CardTitle>
          ...
          <CardContent>
            {selectedReport ? (
              ...
              <div className="text-center text-muted-foreground py-12">
                ...
                Aucun rapport encore. Clique sur « Générer » pour le premier rapport CMO.
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="flex flex-col">
          <CardHeader className="pb-3">
            <CardTitle className="text-base flex items-center gap-2"><Megaphone className="h-4 w-4" /> Chat CMO</CardTitle>
            <CardDescription>Pose une question — il répond avec tes chiffres réels (canaux, conversion, pub, CAC/ROAS)</CardDescription>
          </CardHeader>
          <CardContent className="flex-1 flex flex-col gap-3">
              ...
              <div className="space-y-3">
                {!messages.length && (
                  <div className="text-xs text-muted-foreground space-y-1">
                    ...
                    <div>« Quel canal me rapporte le plus de marge ? »</div>
                    <div>« Ma pub Meta est-elle rentable ? CAC vs marge ? »</div>
                    <div>« Où je perds le plus d'offres dans le funnel ? »</div>
                    <div>« Suis-je trop dépendant d'un seul canal ? »</div>
                  </div>
                )}
                {messages.map((m, i) => (
                ...
                {chatting && (
                  <div className="bg-muted mr-4 rounded-lg p-3 text-sm text-muted-foreground flex items-center gap-2">
                    <RefreshCw className="h-3 w-3 animate-spin" /> Le CMO analyse ton acquisition...
                  </div>
                )}
                <div ref={chatEndRef} />
              </div>
            ...
            <div className="flex gap-2">
              <Input
                placeholder="Ta question au CMO..."
                value={input}
      ...
      {alerts.length > 0 && (
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>Vérification automatique chaque semaine</CardDescription>
          </CardHeader>
