...
const MarginCalculator: React.FC<MarginCalculatorProps> = ({
  ...
  // En mode achat, forcer l'onglet "saleprice"
  const [activeTab, setActiveTab] = useState(isPurchase ? "saleprice" : "monthly");

  // Forcer l'onglet saleprice en mode achat
  useEffect(() => {
    if (isPurchase) {
      setActiveTab("saleprice");
    }
  }, [isPurchase]);
  ...
  // Titre selon le mode
  const cardTitle = isPurchase 
    ? "Calcul de la marge" 
    : (hideFinancialDetails ? "Calculateur de prix" : "Calcul de la marge et mensualité");
  ...
  return (
    <Card className="shadow-sm border-gray-200 rounded-lg">
      ...
      <CardContent className="p-6">
        <Tabs value={activeTab} onValueChange={(value) => !isPurchase && setActiveTab(value)} className="w-full">
          {/* En mode achat, masquer l'onglet mensualité */}
          <TabsList className={cn("grid w-full", isPurchase ? "grid-cols-1" : "grid-cols-2")}>
            {!isPurchase && (
          ...
          {/* Onglet mensualité - uniquement en mode leasing */}
          {!isPurchase && (
          ...
          <TabsContent value="saleprice" className="space-y-5 mt-4">
            ...
            {/* En mode achat, afficher uniquement marge sans mensualité */}
            {isPurchase && targetSalePrice > equipmentPurchasePrice && (
              <div className="grid grid-cols-2 gap-4">
                <div>
            ...
            {/* En mode leasing, afficher tout y compris mensualité */}
            {!isPurchase && !hideFinancialDetails && targetSalePrice > equipmentPurchasePrice && (
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <span className="block text-sm text-gray-700">Prix d'achat :</span>
                  <span className="font-medium">{formatCurrency(equipmentPurchasePrice)}</span>
