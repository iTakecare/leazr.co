...
export const AmbassadorCustomPriceTable: React.FC<AmbassadorCustomPriceTableProps> = ({
  ambassadorId,
}) => {
  ...
  const { data: assignedProducts, isLoading: loadingAssigned } = useQuery({
    queryKey: ['ambassador-assigned-products', ambassadorId, searchTerm],
    queryFn: async () => {
      let query = supabase
        .from('products')
        .select(`
          *,
          ...
          ambassador_custom_prices!inner (
            id,
            margin_rate,
      ...
      return data?.map(product => {
        const customPrice = product.ambassador_custom_prices?.[0];
  ...
  const { data: allProducts, isLoading: loadingAll } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data?.map(product => ({
        ...product,
        isAssigned: product.ambassador_custom_prices?.some(cp => cp.ambassador_id === ambassadorId) || false,
      })) || [];
    },
    enabled: !!ambassadorId && activeTab === "add",
  });
  ...
  const deletePriceMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['ambassador-assigned-products'] });
      queryClient.invalidateQueries({ queryKey: ['all-products-for-ambassador'] });
      toast({
        title: "Prix supprimé",
  ...
  const addProductsMutation = useMutation({
    mutationFn: async (productIds: string[]) => {
      return await addMultipleProductsToAmbassadorCatalog(ambassadorId, productIds, 15);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['ambassador-assigned-products'] });
      queryClient.invalidateQueries({ queryKey: ['all-products-for-ambassador'] });
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle>Catalogue personnalisé ambassadeur</CardTitle>
