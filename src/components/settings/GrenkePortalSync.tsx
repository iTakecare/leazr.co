...
interface ReconcileResponse {
  success: boolean;
  summary?: { total: number; already_linked: number; auto_linked: number; needs_review: number; no_match: number; created_contracts?: number; financed_from_contracts?: number; backfilled_numbers?: number };
  results?: Row[];
  message?: string;
  error?: string;
}
  ...
  const run = async () => {
    ...
    try {
      ...
      const merged: ReconcileResponse = {
        ...
        summary: { total: sum("total"), already_linked: sum("already_linked"), auto_linked: sum("auto_linked"), needs_review: sum("needs_review"), no_match: sum("no_match"), created_contracts: sum("created_contracts"), financed_from_contracts: sum("financed_from_contracts"), backfilled_numbers: sum("backfilled_numbers") },
      ...
      if (merged.success) {
        ...
        const extra = [
          ...
          (s?.backfilled_numbers ?? 0) > 0 ? `${s?.backfilled_numbers} n° demande(s)` : null,
        ].filter(Boolean).join(" · ");
