...
interface PDFModelCompanyInfoProps {
  model: PDFModel;
  onSave: (data: Partial<PDFModel>) => void;
  loading: boolean;
}

const PDFModelCompanyInfo = ({ model, onSave, loading }: PDFModelCompanyInfoProps) => {
  ...
  const form = useForm<PDFModel>({
    defaultValues: {
      name: model.name || "Modèle par défaut",
      companyName: model.companyName || "iTakeCare",
      companyAddress: model.companyAddress || "",
      companyContact: model.companyContact || "",
      companySiret: model.companySiret || "",
      logoURL: model.logoURL || "",
      primaryColor: model.primaryColor || "#2C3E50",
      secondaryColor: model.secondaryColor || "#3498DB",
      headerText: model.headerText || "OFFRE N° {offer_id}",
      footerText: model.footerText || "Cette offre est valable 30 jours à compter de sa date d'émission."
    }
  });
...
export default PDFModelCompanyInfo;
