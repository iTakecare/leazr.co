...
import { dbToAppProduct, dbToAppProducts } from '@/utils/typeMappers';
...
export const getSupabaseClient = () => {
  ...
  if (!supabaseInstance && isBrowser) {
    ...
    // Add interceptors to handle type conversions for products
    const originalFrom = supabaseInstance.from;
    supabaseInstance.from = function(relation: any) {
      const query = originalFrom.call(this, relation);
      
      if (relation === 'products') {
        // Override the then method to map product results
        const originalThen = query.then;
        query.then = function(onfulfilled?: any, onrejected?: any) {
          return originalThen.call(this, (result: any) => {
            if (result && result.data) {
              // Transform products to correct type
              result.data = Array.isArray(result.data) 
                ? dbToAppProducts(result.data) 
                : dbToAppProduct(result.data);
            }
...
export const getAdminSupabaseClient = () => {
  ...
  if (!adminSupabaseInstance && isBrowser) {
    ...
    // Add interceptors to handle type conversions for products
    const originalFrom = adminSupabaseInstance.from;
    adminSupabaseInstance.from = function(relation: any) {
      const query = originalFrom.call(this, relation);
