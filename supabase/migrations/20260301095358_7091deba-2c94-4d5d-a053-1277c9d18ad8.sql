...
BEGIN
  ...
  -- Achats équipements (hors self-leasing - ces contrats sont traités séparément)
  ...
  -- Self-leasing: récupérer les données des contrats
  ...
  -- Self-leasing: calculer CA et achats par mois avec amortissement linéaire
  -- UNIQUEMENT si aucune facture n'existe pour ce contrat ce mois-là
  ...
  SELECT 
    ...
    (COALESCE(lr.total_revenue, 0) + COALESCE(sl.sl_revenue, 0))::numeric as revenue,
    ...
    (COALESCE(lr.total_revenue, 0) + COALESCE(ds.total_sales, 0) + COALESCE(sl.sl_revenue, 0)
     - COALESCE(cnc.total_credit, 0) - COALESCE(ep.total_purchases, 0) - COALESCE(sl.sl_purchase, 0))::numeric as margin,
    CASE 
      WHEN (COALESCE(lr.total_revenue, 0) + COALESCE(ds.total_sales, 0) + COALESCE(sl.sl_revenue, 0)) > 0 
      THEN ROUND((((COALESCE(lr.total_revenue, 0) + COALESCE(ds.total_sales, 0) + COALESCE(sl.sl_revenue, 0)
                   - COALESCE(cnc.total_credit, 0) - COALESCE(ep.total_purchases, 0) - COALESCE(sl.sl_purchase, 0)) 
                  / (COALESCE(lr.total_revenue, 0) + COALESCE(ds.total_sales, 0) + COALESCE(sl.sl_revenue, 0))) * 100)::numeric, 2)
      ELSE 0 
