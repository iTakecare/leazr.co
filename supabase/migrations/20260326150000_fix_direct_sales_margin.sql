...
BEGIN
  ...
  -- NEW: purchase costs for direct sales from offer_equipment
  ...
  SELECT
    ...
    -- purchases now includes direct sales purchase costs (offer_equipment)
    (COALESCE(ep.total_purchases, 0) + COALESCE(sl.sl_purchase, 0) + COALESCE(dsp.total_purchases, 0))::numeric as purchases,
    -- margin deducts direct sales purchase costs
