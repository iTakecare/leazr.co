...
BEGIN
  ...
  -- Self-leasing: use effective start date = LEAST(contract_start_date, first_paid_payment, first_sepa_change)
  -- and latest sepa amount change (converted HTVA) or contract monthly_payment
      ...
      LEAST(
        ...
        COALESCE(
          (SELECT MIN(mpe.created_at)::date FROM mollie_payment_events mpe 
           WHERE mpe.contract_id = c.id AND mpe.status = 'paid' AND mpe.amount > 1),
          COALESCE(c.contract_start_date, c.created_at::date)
        ),
        COALESCE(
          (SELECT MIN(msc.created_at)::date FROM mollie_sepa_changes msc 
           WHERE msc.contract_id = c.id AND msc.change_type IN ('start_date', 'amount')),
          COALESCE(c.contract_start_date, c.created_at::date)
        )
      ) as start_date,
      c.contract_end_date as end_date,
      -- Use latest sepa amount change (HTVA = TVAC / 1.21) or contract monthly_payment
      COALESCE(
  ...
  self_leasing_by_month AS (
    SELECT
      ...
      SUM(
        CASE 
          -- Use month-level comparison: contract active if target month >= start month
          WHEN (target_year * 12 + m.month_num) >= (EXTRACT(YEAR FROM slc.start_date)::integer * 12 + EXTRACT(MONTH FROM slc.start_date)::integer)
          ...
          THEN slc.effective_monthly_payment 
          ELSE 0 
        END
      ) as sl_revenue,
      SUM(
