-- Supprimer la contrainte unique sur domain car plusieurs entreprises peuvent utiliser le même domaine avec des sous-domaines différents
ALTER TABLE public.company_domains DROP CONSTRAINT IF EXISTS company_domains_domain_key;

-- Ajouter une contrainte unique sur (domain, subdomain) à la place
ALTER TABLE public.company_domains ADD CONSTRAINT company_domains_domain_subdomain_key UNIQUE (domain, subdomain);

-- Insérer des domaines par défaut pour les entreprises existantes avec des sous-domaines uniques
INSERT INTO public.company_domains (company_id, domain, subdomain, is_primary, is_active)
SELECT 
  id as company_id,
  'leazr.co' as domain,
  LOWER(REPLACE(REPLACE(name, ' ', ''), '''', '')) as subdomain,
  true as is_primary,
  true as is_active
