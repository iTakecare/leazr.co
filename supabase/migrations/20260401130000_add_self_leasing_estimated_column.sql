-- Séparer le revenu self-leasing RÉELLEMENT facturé de l'estimé (CTE)
-- Pour le mois en cours, le dashboard ne doit afficher que le facturé dans le CA réel,
-- et montrer l'estimé en "(Prév. : X €)" tant que la facture n'est pas émise.
...
RETURNS TABLE(
  ...
  self_leasing_estimated numeric,   -- nouveau : part estimée (non encore facturée)
  purchases numeric,
  margin numeric,
...
BEGIN
  ...
  self_leasing_by_month AS (
    SELECT
      ...
      -- Revenus estimés uniquement pour les mois PASSÉS (< mois courant)
      -- Pour le mois en cours et les suivants → exposé séparément en self_leasing_estimated
      SUM(
        CASE
          WHEN make_date(target_year, m.month_num, 1) >= slc.start_date
               ...
               -- Uniquement pour les mois passés (antérieurs au mois courant)
               AND make_date(target_year, m.month_num, 1) < date_trunc('month', CURRENT_DATE)
          THEN slc.monthly_payment
          ELSE 0
        END
      ) as sl_revenue,
      -- Même logique pour les achats amortis self-leasing
      SUM(
        CASE
          WHEN make_date(target_year, m.month_num, 1) >= slc.start_date
               AND (slc.end_date IS NULL OR make_date(target_year, m.month_num, 1) <= slc.end_date)
      ...
      -- Revenus estimés pour le mois en cours ET les mois futurs (→ prévisionnel seulement)
      SUM(
        CASE
          WHEN make_date(target_year, m.month_num, 1) >= slc.start_date
               AND (slc.end_date IS NULL OR make_date(target_year, m.month_num, 1) <= slc.end_date)
               ...
               -- Mois courant et futurs uniquement
               AND make_date(target_year, m.month_num, 1) >= date_trunc('month', CURRENT_DATE)
          THEN slc.monthly_payment
          ELSE 0
        END
  ...
  SELECT
    ...
    -- self_leasing_revenue = factures réelles + estimé mois PASSÉS seulement
    (COALESCE(sl.sl_revenue, 0) + COALESCE(sli.total_invoiced, 0))::numeric as self_leasing_revenue,
    -- self_leasing_estimated = estimé mois COURANT et FUTURS (affiché en prévisionnel)
    ...
    -- La marge inclut l'estimé passé ET le prévisionnel pour les mois futurs (vision complète)
    (COALESCE(lr.total_revenue, 0) + COALESCE(ds.total_sales, 0)
     + COALESCE(sl.sl_revenue, 0) + COALESCE(sli.total_invoiced, 0) + COALESCE(sl.sl_estimated, 0)
    ...
    CASE
      WHEN (COALESCE(lr.total_revenue, 0) + COALESCE(ds.total_sales, 0)
            + COALESCE(sl.sl_revenue, 0) + COALESCE(sli.total_invoiced, 0) + COALESCE(sl.sl_estimated, 0)) > 0
      THEN ROUND((((COALESCE(lr.total_revenue, 0) + COALESCE(ds.total_sales, 0)
                  ...
                  / (COALESCE(lr.total_revenue, 0) + COALESCE(ds.total_sales, 0)
                     + COALESCE(sl.sl_revenue, 0) + COALESCE(sli.total_invoiced, 0) + COALESCE(sl.sl_estimated, 0))) * 100)::numeric, 2)
      ELSE 0
