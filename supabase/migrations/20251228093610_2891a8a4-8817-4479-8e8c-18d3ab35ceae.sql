-- Fix ambiguous/broken public RPC used by public signature + signed PDF download pages
-- 1) Remove the broken TEXT overload (it references non-existent columns like signature_token)
DROP FUNCTION IF EXISTS public.get_contract_for_signature(text);

-- 2) Replace the UUID version with a definition that matches the current schema
...
-- Ensure public pages (anon) can call the RPC
