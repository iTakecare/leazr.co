-- Recréer la fonction get_contract_statistics_by_status(p_year) 
-- Pour pending/refused : ignorer le filtre d'année car ce sont des stocks en cours
...
BEGIN
    ...
    -- Section PENDING : TOUTES les offres en attente (stock actuel, pas de filtre d'année)
    SELECT 
    ...
    WHERE o.company_id = user_company_id
        ...
        -- PAS de filtre d'année pour pending : c'est le stock actuel
    
    UNION ALL
    
    -- Section REFUSED : TOUTES les offres refusées (stock actuel, pas de filtre d'année)
    SELECT 
    ...
    WHERE o.company_id = user_company_id
        ...
        -- PAS de filtre d'année pour refused : c'est le stock actuel
    
    UNION ALL
    
    -- Section REALIZED : CA réalisé basé sur les factures leasing (AVEC filtre année)
    SELECT 
    ...
    -- Section DIRECT_SALES : Ventes directes basées sur les factures purchase (AVEC filtre année)
    SELECT 
    ...
    -- Section FORECAST : CA prévisionnel basé sur les contrats actifs (AVEC filtre année)
    SELECT 
