-- Corriger le coefficient Grenke de 3.1602 à 3.16
DROP FUNCTION IF EXISTS public.recalculate_offer_to_target_monthly(UUID, NUMERIC);

-- Fonction corrigée avec le bon coefficient Grenke 3.16
...
DECLARE
  ...
  global_coefficient NUMERIC := 3.16; -- Coefficient Grenke correct de la DB
...
BEGIN
  -- Calculer le montant financé cible à partir de la mensualité avec le bon coefficient
  ...
  LOOP
    ...
    SET 
      selling_price = new_selling_price_calc / equipment_record.quantity,
      monthly_payment = new_monthly_payment_calc / equipment_record.quantity,
...
-- Recalculer l'offre avec le coefficient correct 3.16
