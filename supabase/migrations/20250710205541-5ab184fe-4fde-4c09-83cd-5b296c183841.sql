-- Migration finale d'optimisation RLS - Élimination des warnings restants

-- Phase 1: Optimiser les politiques offers
...
-- Phase 3: Optimiser les politiques clients
...
-- Phase 5: Optimiser les politiques chat
...
-- Phase 6: Optimiser offer_equipment
...
-- Phase 8: Optimiser contract_workflow_logs
...
-- Phase 9: Optimiser offer_documents
...
-- Phase 10: Optimiser offer_equipment_attributes
...
-- Phase 11: Optimiser offer_equipment_specifications
...
-- Phase 12: Optimiser offer_info_requests
...
-- Phase 13: Optimiser offer_notes
...
-- Phase 14: Optimiser offer_workflow_logs
...
-- Phase 15: Optimiser pdf_templates
...
-- Phase 16: Optimiser prospects (si la table existe)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'prospects') THEN
    ...
    EXECUTE 'CREATE POLICY "prospects_optimized" ON public.prospects FOR ALL USING (is_admin_optimized())';
  END IF;
END $$;

-- Phase 17: Optimiser smtp_settings (si la table existe)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'smtp_settings') THEN
    ...
    EXECUTE 'CREATE POLICY "smtp_settings_optimized" ON public.smtp_settings FOR ALL USING (company_id = get_user_company_id() OR is_admin_optimized()) WITH CHECK (company_id = get_user_company_id() OR is_admin_optimized())';
  END IF;
END $$;

-- Phase 18: Créer des index standards pour optimiser les performances
CREATE INDEX IF NOT EXISTS idx_offers_company_user 
ON offers(company_id, user_id);

CREATE INDEX IF NOT EXISTS idx_clients_company_user 
ON clients(company_id, user_id);

CREATE INDEX IF NOT EXISTS idx_contracts_company_user 
