CREATE OR REPLACE FUNCTION public.get_monthly_financial_data(p_year integer)
 RETURNS TABLE(month_name text, month_number integer, year integer, revenue numeric, direct_sales_revenue numeric, self_leasing_revenue numeric, self_leasing_estimated numeric, purchases numeric, margin numeric, margin_percentage numeric, contracts_count integer, offers_count integer, credit_notes_amount numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
