-- CRITICAL SECURITY FIXES - Remove public access from sensitive data (Fixed)
...
DROP POLICY IF EXISTS "company_email_confirmations_secure_access" ON public.company_email_confirmations;
...
DROP POLICY IF EXISTS "prospects_saas_admin_only" ON public.prospects;
...
DROP POLICY IF EXISTS "offer_upload_links_company_secure" ON public.offer_upload_links;
...
DROP POLICY IF EXISTS "content_cms_public_safe_read" ON public.content_cms;

-- Keep only safe public content readable
CREATE POLICY "content_cms_secure_public_read" 
...
-- Only allow server-side access to tokens
