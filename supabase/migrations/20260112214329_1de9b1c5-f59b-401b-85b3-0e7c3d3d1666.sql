-- Align get_monthly_financial_data RPC output with frontend expectations (month_name, month_number, ...)
...
BEGIN
  ...
  SELECT
    ...
    target_year as year,
    -- revenue MUST be NET for the dashboard (gross - credit notes)
    ...
    ((COALESCE(li.gross_revenue, 0) - COALESCE(cn.total_credit_notes, 0)) - COALESCE(ep.total_purchases, 0))::numeric as margin,
    CASE
      WHEN COALESCE(li.gross_revenue, 0) > 0
      THEN ((((COALESCE(li.gross_revenue, 0) - COALESCE(cn.total_credit_notes, 0)) - COALESCE(ep.total_purchases, 0)) / COALESCE(li.gross_revenue, 1)) * 100)::numeric
      ELSE 0::numeric
