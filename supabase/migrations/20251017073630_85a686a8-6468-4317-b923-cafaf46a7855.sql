-- Fix dashboard calculations v2: Use correct column structure
-- contract_equipment has: purchase_price, margin, quantity (NO selling_price)
-- offer_equipment has: purchase_price, margin, quantity, selling_price

-- 1. Fix get_monthly_financial_data function
...
BEGIN
    ...
    contract_financials AS (
        SELECT 
            ...
            COALESCE(SUM(ce.purchase_price * (1 + COALESCE(ce.margin, 0) / 100) * ce.quantity), 0) as total_selling
        FROM contracts c
            ...
            COALESCE(
                SUM(
                    CASE 
                        ...
                        ELSE oe.purchase_price * (1 + COALESCE(oe.margin, 0) / 100) * oe.quantity
                    END
                ), 0
            ) as total_selling
        FROM offers o
...
-- 2. Fix get_contract_statistics_by_status function
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(SUM(
            (SELECT SUM((ce.purchase_price * (1 + COALESCE(ce.margin, 0) / 100) - ce.purchase_price) * ce.quantity)
             FROM contract_equipment ce WHERE ce.contract_id = c.id)
        ), 0) as total_margin
    FROM contracts c
    WHERE c.company_id = user_company_id
    ...
    SELECT 
        ...
        COALESCE(SUM(
            (SELECT SUM(
                (CASE 
                    WHEN oe.selling_price IS NOT NULL 
                    THEN oe.selling_price
                    ELSE oe.purchase_price * (1 + COALESCE(oe.margin, 0) / 100)
                END - oe.purchase_price) * oe.quantity
             ) FROM offer_equipment oe WHERE oe.offer_id = o.id)
        ), 0) as total_margin
    FROM offers o
    WHERE o.company_id = user_company_id
...
-- 3. Fix get_company_dashboard_metrics function
