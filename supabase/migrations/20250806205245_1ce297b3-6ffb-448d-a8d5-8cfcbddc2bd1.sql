-- Supprimer l'ancienne fonction pour pouvoir la recréer avec le bon type de retour
DROP FUNCTION IF EXISTS public.get_template_for_offer(uuid, text, text);

-- Recréer la fonction avec la bonne casse pour companyName
...
BEGIN
  ...
  SELECT 
    ...
    jsonb_build_object(
      'id', c.id,
      'name', c."companyName",  -- Utiliser la bonne casse avec guillemets
      'address', c.address,
      'city', c.city,
