-- Fix search_path security issue in create_api_key_secure function
...
BEGIN
  ...
  current_user_company_id := public.get_user_company_id();
  ...
  generated_api_key := generated_api_key || replace(replace(replace(encode(decode(replace(public.gen_random_uuid()::text, '-', ''), 'hex'), 'base64'), '/', '_'), '+', '-'), '=', '');
  generated_api_key := generated_api_key || replace(replace(replace(encode(decode(replace(public.gen_random_uuid()::text, '-', ''), 'hex'), 'base64'), '/', '_'), '+', '-'), '=', '');
