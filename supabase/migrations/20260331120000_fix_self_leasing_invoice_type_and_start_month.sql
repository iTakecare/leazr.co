-- Fix: Self-leasing invoices manquantes dans CA Self-Leasing
--
-- Problème 1 (données) :
--   Les factures créées pour les contrats self-leasing (ex: LOC-ITC-2026-03003 Eric Bruyr)
--   n'ont pas billing_data->>'type' = 'self_leasing_monthly' → comptées dans CA Leasing
--   au lieu de CA Self-Leasing, ET le NOT EXISTS supprime aussi la mensualité calculée.
--
-- Problème 2 (calcul) :
--   Dans self_leasing_by_month, la condition
--     make_date(target_year, m.month_num, 1) >= slc.start_date
--   exclut le mois de démarrage quand le contrat commence en cours de mois
--   (ex: start_date = 2026-03-26 → mars exclu car 2026-03-01 < 2026-03-26).
--   La condition correcte est de comparer au 1er du mois de start_date.

-- ── 1. Corriger les factures existantes ─────────────────────────────────────
UPDATE invoices i
SET billing_data = jsonb_set(
  COALESCE(i.billing_data, '{}'::jsonb),
  '{type}',
  '"self_leasing_monthly"'
)
WHERE i.invoice_type = 'leasing'
...
-- ── 2. Corriger la fonction get_monthly_financial_data ───────────────────────
...
BEGIN
  ...
  self_leasing_by_month AS (
    SELECT
      ...
      SUM(
        CASE
          -- Fix: comparer au 1er du mois de start_date (pas au jour exact)
          -- pour inclure le mois de démarrage même si le contrat commence en cours de mois
          WHEN make_date(target_year, m.month_num, 1) >= date_trunc('month', slc.start_date)::date
      ...
      SUM(
        CASE
          -- Fix: même correction pour les achats (cohérence)
          WHEN make_date(target_year, m.month_num, 1) >= date_trunc('month', slc.start_date)::date
               AND (slc.end_date IS NULL OR make_date(target_year, m.month_num, 1) <= slc.end_date)
               AND (
                 ((EXTRACT(YEAR FROM make_date(target_year, m.month_num, 1)) - EXTRACT(YEAR FROM slc.start_date)) * 12
