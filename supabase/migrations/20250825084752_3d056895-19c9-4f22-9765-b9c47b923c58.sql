...
-- 4. Add missing RLS policies for tables that have RLS enabled but incomplete policies

-- Company Module Changes - Admin only access
CREATE POLICY "company_module_changes_admin_only" 
ON public.company_module_changes
FOR ALL
USING (is_saas_admin())
WITH CHECK (is_saas_admin());

-- Content CMS - Secure write access
