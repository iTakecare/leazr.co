-- Step 1: Update contract_equipment to set actual_purchase_date for the original 2025 contract
UPDATE contract_equipment
SET actual_purchase_date = '2025-12-15'
WHERE contract_id = 'bd2026f1-ccdd-4d87-8cc3-9d940bb8063b';

-- Step 2: Drop and recreate the function with correct return type
...
BEGIN
  ...
  credit_notes_data AS (
    SELECT 
      ...
      SUM(i.total_amount) as credit_notes_total
    FROM invoices i
    WHERE i.company_id = user_company_id
      AND i.type = 'credit_note'
      AND EXTRACT(YEAR FROM i.issued_at) = target_year
    GROUP BY EXTRACT(MONTH FROM i.issued_at), EXTRACT(YEAR FROM i.issued_at)
  ),
  ...
  invoice_financials AS (
    SELECT 
      EXTRACT(MONTH FROM i.issued_at) as month,
      EXTRACT(YEAR FROM i.issued_at) as year,
      SUM(CASE WHEN i.type = 'standard' THEN i.total_amount ELSE 0 END) as total_revenue,
      -- Purchases only for invoices WITHOUT a contract (direct sales)
      SUM(CASE 
        WHEN i.type = 'standard' AND i.contract_id IS NULL THEN
          COALESCE((
            SELECT SUM(
