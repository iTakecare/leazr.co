-- Fix: get_monthly_financial_data — duplication due to LEFT JOIN on invoices
--
-- Bug: LEFT JOIN invoices inv ON inv.contract_id = c.id created N rows per
-- contract_equipment item (N = number of invoices for that contract).
-- For a 36-month Grenke contract with 24 invoices, each equipment item was
-- counted 24×. Setting actual_purchase_date made all 24 rows land in the
-- same year, causing massive over-counting ("le bordel dans chaque année").
--
-- Fix: Replace the unbounded LEFT JOIN with a subquery that returns one row
-- per contract (the MIN invoice_date). Same fallback logic, no duplication.
...
BEGIN
  ...
  -- purchase costs for direct sales from offer_equipment
  ...
  -- FIX: Use a subquery to get ONE invoice date per contract (MIN),
  -- instead of LEFT JOIN which created one row per invoice per equipment item.
  contract_first_invoice AS (
    SELECT contract_id, MIN(invoice_date) as first_invoice_date
    FROM invoices
  ...
  equipment_purchases_by_month AS (
    ...
    LEFT JOIN contract_first_invoice cfi ON cfi.contract_id = c.id
    WHERE c.company_id = user_company_id
      ...
      AND c.status IN ('signed', 'active', 'delivered', 'completed', 'equipment_ordered', 'extended')
  ...
  SELECT
    ...
    (COALESCE(ep.total_purchases, 0) + COALESCE(sl.sl_purchase, 0) + COALESCE(dsp.total_purchases, 0))::numeric as purchases,
