-- Fix: la colonne "purchases" (Achats) omettait le coût d'achat des ventes directes (dsp)
-- alors que la marge le déduisait déjà. Résultat : dans le tableau mensuel,
-- CA − notes de crédit − Achats ≠ Marge (écart = achats des ventes directes).
--
-- Correction : ajouter dsp.total_purchases à la colonne purchases pour qu'elle
-- soit cohérente avec le calcul de la marge. Aucun autre changement.
...
BEGIN
  ...
  self_leasing_by_month AS (
    SELECT
      ...
      -- sl_purchase : coût amorti uniquement si le contrat est actif ce mois
      SUM(
        CASE
      ...
      -- sl_estimated : revenus attendus sans facture (prévisionnel)
      SUM(
        CASE
          WHEN make_date(target_year, m.month_num, 1) >= slc.start_date
               AND (slc.end_date IS NULL OR make_date(target_year, m.month_num, 1) <= slc.end_date)
  ...
  SELECT
    ...
    -- Estimé = revenus attendus sans facture (prévisionnel)
    COALESCE(sl.sl_estimated, 0)::numeric as self_leasing_estimated,
    -- Achats : équipements leasing + sl_purchase (si facture SL ce mois) + achats des ventes directes (dsp)
    -- FIX : dsp était omis ici alors qu'il est déduit dans la marge → colonne Achats sous-estimée.
    (COALESCE(ep.total_purchases, 0)
     + CASE WHEN COALESCE(sli.total_invoiced, 0) > 0 THEN COALESCE(sl.sl_purchase, 0) ELSE 0 END
     + COALESCE(dsp.total_purchases, 0)
    )::numeric as purchases,
    -- Marge : sl_purchase déduit UNIQUEMENT si une facture SL existe ce mois
