-- Recréer la fonction get_monthly_financial_data avec la logique : 
-- Achats basés sur contract_start_date + purchase_price (pas de fallback)
...
BEGIN
  ...
  -- NOUVELLE LOGIQUE : Achats basés sur contract_start_date + purchase_price
  -- Quand un contrat démarre, les équipements sont considérés comme achetés
  equipment_purchases_by_month AS (
    SELECT
      EXTRACT(MONTH FROM c.contract_start_date)::integer as month,
      SUM(ce.purchase_price * COALESCE(ce.quantity, 1)) as total_purchases
    FROM contract_equipment ce
    JOIN contracts c ON c.id = ce.contract_id
    WHERE c.company_id = user_company_id
      AND COALESCE(c.is_self_leasing, false) = false
      AND ce.purchase_price IS NOT NULL
      AND ce.purchase_price > 0
