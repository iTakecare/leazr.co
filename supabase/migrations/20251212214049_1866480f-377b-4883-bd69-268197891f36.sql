-- Fix: Allow signing contracts with either 'draft' or 'pending_signature' status
...
DECLARE
  ...
  v_offer_id UUID;
BEGIN
  -- Get contract ID - Accept both 'draft' and 'pending_signature' statuses
  ...
  WHERE contract_signature_token = p_signature_token::uuid
    ...
    AND signature_status IN ('pending_signature', 'draft');
