-- Recréer immédiatement le modèle document_request manquant avec les bonnes colonnes
...
Nous avons besoin de documents supplémentaires pour traiter votre demande de leasing avec {{company_name}}.
...
Veuillez utiliser le lien suivant pour télécharger vos documents :
{{upload_link}}
...
L''équipe {{company_name}}',
  ...
  now(),
  now()
) ON CONFLICT (type, company_id) DO UPDATE SET
  name = EXCLUDED.name,
