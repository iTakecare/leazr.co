-- Fix get_monthly_financial_data function: correct column references and credit notes source
...
BEGIN
  ...
  -- Invoice revenue (excluding credit notes which are in separate table)
  invoice_revenue AS (
    SELECT
  ...
  -- Credit notes from credit_notes table
  ...
  -- Self-leasing monthly purchases
  ...
  active_contracts_count AS (
    ...
    LEFT JOIN contracts c ON c.company_id = user_company_id
      ...
      AND c.created_at <= make_date(target_year, m.m, 1) + interval '1 month' - interval '1 day'
      AND (c.end_date IS NULL OR c.end_date >= make_date(target_year, m.m, 1))
    GROUP BY m.m
  )
  SELECT
