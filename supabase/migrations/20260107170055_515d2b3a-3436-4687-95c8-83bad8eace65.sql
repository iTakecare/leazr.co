-- Fix get_monthly_financial_data(p_year): remove non-existent invoices.total_purchase_price
-- and reuse robust purchases logic similar to get_monthly_financial_data() while keeping self-leasing monthly logic.
...
BEGIN
  ...
  -- Invoice-based financials (exclude self-leasing contract invoices to avoid double counting)
