// Contenus localisés des emails commerciaux de leasing (acceptation / refus),
// partagés par les edge functions. Les chaînes DOIVENT rester synchronisées avec
// le module frontend src/lib/leasingEmailContent.ts (aperçu des modales), afin que
// ce que l'admin prévisualise corresponde à ce qui part réellement.
...
const ACCEPT: Record<Lang, AcceptStrings> = {
  fr: {
...
export function acceptanceSubject(lang: Lang): string {
  return (ACCEPT[lang] ?? ACCEPT.fr).subject;
}

export function buildAcceptanceHtml(lang: Lang, clientFirstName: string, equipmentListHtml: string): string {
...
export function buildAcceptanceText(lang: Lang, clientFirstName: string, equipmentTextList: string): string {
  const t = ACCEPT[lang] ?? ACCEPT.fr;
  const greetingPlain = t.greeting(clientFirstName).replace(/<[^>]+>/g, "");
  return `${greetingPlain}
...
// ───────────────────────────────── REFUS ─────────────────────────────────
export function rejectionTitle(lang: Lang): string {
  switch (lang) {
...
export function rejectionBody(lang: Lang): string {
  switch (lang) {
...
// ───────────────────────── CLÔTURE SANS SUITE (Score D) ─────────────────────────
export function noFollowUpSubject(lang: Lang): string {
  switch (lang) {
...
export function noFollowUpBody(lang: Lang, clientName: string): string {
  const n = clientName || "Client";
  switch (lang) {
    case "nl": return `<p>Beste ${n},</p>
...
<p>Kind regards,<br/>The iTakecare team</p>`;
    case "de": return `<p>Guten Tag ${n},</p>
...
export function buildNoFollowUpHtml(title: string, bodyHtml: string): string {
  return `
<!DOCTYPE html>
<html>
<head>
...
export function buildRejectionHtml(title: string, body: string): string {
...
<head>
  ...
  <style>
    ...
    .header h1 { margin: 0; font-size: 24px; }
    .content { background: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; white-space: pre-wrap; }
    .footer { text-align: center; color: #6b7280; font-size: 14px; margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb; }
  </style>
</head>
<body>
