...
function buildSystem(companyId: string): string {
  ...
  return (
    ...
    `- clients (fiche client : name, company, status, created_at...), contracts (contract_number, status, is_self_leasing, monthly_payment, contract_duration, leaser_name, client_name, offer_id, created_at), contract_equipment, contract_workflow_logs (même principe que offer_workflow_logs).\n` +
    `- ATTENTION rentabilité : pour un contrat is_self_leasing = true, le coût d'achat est LISSÉ sur la durée (marge incluse dans chaque mensualité) → marge = monthly_payment × contract_duration − coût d'achat − commission (marge projetée). Ne JAMAIS conclure à une perte en comparant le facturé à ce jour au coût d'achat total. Pour les contrats refinancés (is_self_leasing = false), marge réalisée = facturé − coût − commission.\n` +
