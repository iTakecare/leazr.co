...
import { noFollowUpSubject, noFollowUpBody, buildNoFollowUpHtml } from "../_shared/leasingEmails.ts";
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const emailTitle = customTitle || noFollowUpSubject(lang);

    // Remplacer {{client_name}} dans le contenu personnalisé
    let emailBody = customContent || noFollowUpBody(lang, offer.client_name || 'Client');
    emailBody = emailBody.replace(/\{\{client_name\}\}/g, offer.client_name || 'Client');

    // Générer le HTML de l'email avec un header gris/neutre
    const htmlContent = buildNoFollowUpHtml(emailTitle, emailBody);
